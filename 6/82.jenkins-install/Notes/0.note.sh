Docker compose here will create two containers i.e jenkins-docker & my-jenkins
In this container the docker cli will get installed my-jenkins
docker-daemon will run on the jenkins-docker
...
TLS protocl is on TCP network ... To secure the network between the My-jenkins & the jenkins-docker we need to secure connection which 
TLS  provide this one.
